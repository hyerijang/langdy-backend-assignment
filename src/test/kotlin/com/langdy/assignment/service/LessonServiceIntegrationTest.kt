...
class LessonServiceIntegrationTest : IntegrationTestBase() {
    ...
    fun `concurrent booking - same teacher multiple students only one succeeds`() {
        ...
        val courseId = requireNotNull(course.id) { "course id must not be null" }
        val teacherId = requireNotNull(teacher.id) { "teacher id must not be null" }

        for (s in students) {
            executor.submit {
                try {
                    startLatch.await()
                    val req = LessonRequest(startAt = startAt, courseId = courseId, teacherId = teacherId)
                    val resp =
                        lessonService.createLesson(
                            req,
                            requireNotNull(s.id) { "student id must not be null" },
                        )
    ...
    fun `concurrent booking - same student multiple teachers only one succeeds`() {
        ...
        val courseId = requireNotNull(course.id) { "course id must not be null" }
        val studentId = requireNotNull(student.id) { "student id must not be null" }

        for (t in teachers) {
            val teacherId = requireNotNull(t.id) { "teacher id must not be null" }
            executor.submit {
                try {
                    startLatch.await()
                    val req = LessonRequest(startAt = startAt, courseId = courseId, teacherId = teacherId)
                    val resp = lessonService.createLesson(req, studentId)
                    successes.add(resp.id)
                } catch (e: Throwable) {
                    errors.add(e)
                } finally {
